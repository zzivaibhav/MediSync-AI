...
resource "aws_security_group" "backend-load_balancer" {
   name = "backend_lb-sg"
    description = "Allows HTTP and HTTPS traffic from the backend frontend only."
    vpc_id = aws_vpc.vpc.id  
    tags = {
        Name = "backedn_lb-sg"
    }
    ...
    ingress {
        ...
        to_port     = 80
        protocol    = "tcp"
        security_groups = [aws_security_group.frontend.id, aws_security_group.application-load_balancer.id]
    }
